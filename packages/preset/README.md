...
The presets the deployment ships live in [`apps/cli/config/agent-presets/`](../../apps/cli/config/agent-presets) — one directory each, and that directory listing is the discovered inventory. A deployment may publish only selected ids to roster surfaces; naming the shipped ids here too would still be a second list to keep in step, and the first one to fall behind.
