...
`conversation.hero.brand` 是 Hero 品牌视觉的根作用域 single slot。注册项持有标记、标语与徽章，会话壳则保留它们的位置以及常驻 Hero／编辑器树；`HeroBrandOwnerProps` 不携带业务数据。内置产品 Hero 以 priority `0` 注册；皮肤可以用 `-100` 等数值更低的 priority 注册来遮蔽它，dispose（资源释放）该注册后会在不重挂会话壳、Hero 外壳或编辑器的情况下重新显示默认 Hero（[决策](../../../.agents/notes/implemented/feature/2026-08-18-root-scoped-brand-slots.md)）。
