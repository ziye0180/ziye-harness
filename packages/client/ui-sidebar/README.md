...
`sidebar.brand` is a root-scoped single slot inside the shell's existing brand buttons. The host passes `variant: 'wordmark' | 'mark'`; the occupant owns only the visual identity, while the shell retains the New Session and collapse click targets and their accessible labels. The built-in product brand registers at priority `0`. A skin can register at a lower numeric priority such as `-100` to shadow it, and disposing that registration reveals the default without remounting the sidebar ([decision](../../../.agents/notes/implemented/feature/2026-08-18-root-scoped-brand-slots.md)).
