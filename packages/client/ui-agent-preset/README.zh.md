...
发布的 roster 为空时，默认设置行、新会话 chip 与管理分区均不渲染。该情况既包括没有 roster 服务的部署，也包括可见性策略刻意不发布任何选项的部署；若会话记录的是隐藏 preset，只读头部标签仍显示其 id。非空的过滤名单只暴露已发布行，因此隐藏的用户 preset 需直接通过文件管理，或等待部署重新发布后再从界面管理。未配置可写根目录的部署返回 `authorable: false`，分区随之退化为只读浏览：可见的随附组装仍可在查看器中打开，但每个复制操作都被禁用并以原因作提示，而不是给出一个创建必然失败的对话框。
