...
纯 React 原子组件（零 cordis）：StateDot、DisclosureRow、由 Lucide 支撑的图标兼容集、Button/Pill/Menu/Modal/Input、Toast 短时横幅、OnboardingSurface 首次使用接管层（portal 到 body 的遮罩加不透明展示层，在且仅在自身生命周期内保持 `#root` 为 `inert`）、markdown 家族（MessageText/MarkdownText/JsonBlock）、只读 JsonTree 检查器、`useAnchoredMaxHeight` 钩子（把底部锚定的浮层高度收敛到锚点上方的视口空间，并在 resize、scroll 与调用方提供的依赖变化时重新测量）、`useAnchoredPosition` 钩子（让固定定位的浮动面板跟住锚点：测量、偏移、按视口边距钳制，并在捕获阶段滚动、窗口缩放与面板自身尺寸变化时重新定位）、TerminalBlock、DiffBlock、ReadBlock、SearchBlock，以及 WebBlock。图标层保留全部 70 个公开 `Icon*` 名称及其 `{ size, className }` props：69 个语义 glyph 使用静态导入的 Lucide 组件，8×10 的会话树连接器则保留精确的产品几何。
...
- **Lucide 映射保留含义，不复刻原 Figma 像素**：旧导出名与默认尺寸保持稳定，但 69 个语义 glyph 改用 Lucide 的圆角描边几何。产品品牌图形与会话树连接器继续使用自有 SVG，因为拿通用库 glyph 替换会改变其身份或布局几何。
